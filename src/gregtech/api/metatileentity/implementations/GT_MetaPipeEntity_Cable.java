...
public class GT_MetaPipeEntity_Cable extends MetaPipeEntity implements IMetaTileEntityCable {
	...
	public long injectEnergyUnits(byte aSide, long aVoltage, long aAmperage) {
		if (!getBaseMetaTileEntity().getCoverBehaviorAtSide(aSide).letsEnergyIn(aSide,
			...
			return 0;
		return transferElectricity(aSide, aVoltage, aAmperage,
				new ArrayList<TileEntity>(Arrays.asList((TileEntity) getBaseMetaTileEntity())));
	}
	...
	public long transferElectricity(byte aSide, long aVoltage, long aAmperage,
		...
		if (aVoltage > 0)
			for (byte i = 0; i < 6 && aAmperage > rUsedAmperes; i++)
				if (i != aSide && (mConnections & (1 << i)) != 0
						&& getBaseMetaTileEntity().getCoverBehaviorAtSide(i).letsEnergyOut(i,
								getBaseMetaTileEntity().getCoverIDAtSide(i),
								getBaseMetaTileEntity().getCoverDataAtSide(i), getBaseMetaTileEntity())) {
					...
					if (!aAlreadyPassedTileEntityList.contains(tTileEntity)) {
						...
						if (tTileEntity instanceof IEnergyConnected) {
							...
							if (((IEnergySink) tTileEntity).acceptsEnergyFrom((TileEntity) getBaseMetaTileEntity(),
									tDirection)) {
							 ...
							 * tDirection =
							 * ForgeDirection.getOrientation(i).getOpposite();
							 * int rfOut = (int) (aVoltage *
							 * GregTech_API.mEUtoRF / 100); if
							 * (((IEnergyReceiver)
							 * tTileEntity).receiveEnergy(tDirection, rfOut,
							 * true) == rfOut) { ((IEnergyReceiver)
							 * tTileEntity).receiveEnergy(tDirection, rfOut,
							 * false); rUsedAmperes++; } else if
							 * (((IEnergyReceiver)
							 * tTileEntity).receiveEnergy(tDirection, rfOut,
							 * true) > 0) { if (mRestRF == 0) { int RFtrans =
							 * ((IEnergyReceiver)
							 * tTileEntity).receiveEnergy(tDirection, (int)
							 * rfOut, false); rUsedAmperes++; mRestRF = rfOut -
							 * RFtrans; } else { int RFtrans =
							 * ((IEnergyReceiver)
							 * tTileEntity).receiveEnergy(tDirection, (int)
							 * mRestRF, false); mRestRF = mRestRF - RFtrans; } }
							 * if (GregTech_API.mRFExplosions &&
							 * ((IEnergyReceiver)
							 * tTileEntity).getMaxEnergyStored(tDirection) <
							 * rfOut * 600) { if (rfOut > 32 *
							 * GregTech_API.mEUtoRF / 100)
			...
			if (aTick % 20 == 0) {
				...
				for (byte i = 0, j = 0; i < 6; i++) {
					...
					if (aBaseMetaTileEntity.getCoverBehaviorAtSide(i).alwaysLookConnected(i,
						...
						/*
						 * if (GregTech_API.mOutputRF && tTileEntity instanceof
						 * IEnergyReceiver && ((IEnergyReceiver)
						 * tTileEntity).canConnectEnergy(ForgeDirection.
						 * getOrientation(j))) { mConnections |= (1 << i);
						 * continue; }
						 */
						/*
