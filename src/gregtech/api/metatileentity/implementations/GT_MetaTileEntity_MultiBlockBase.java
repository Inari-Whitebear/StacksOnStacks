...
public abstract class GT_MetaTileEntity_MultiBlockBase extends MetaTileEntity {
	...
	public boolean mMachine = false, mWrench = false, mScrewdriver = false, mSoftHammer = false, mHardHammer = false,
			mSolderingTool = false, mCrowbar = false;
	...
	public GT_MetaTileEntity_MultiBlockBase(int aID, String aName, String aNameRegional) {
		...
		this.disableMaintenance = GregTech_API.sMachineFile.get(ConfigCategories.machineconfig,
	...
	public void onPostTick(IGregTechTileEntity aBaseMetaTileEntity, long aTick) {
		if (aBaseMetaTileEntity.isServerSide()) {
								...
								if (mMaxProgresstime > 0 && ++mProgresstime >= mMaxProgresstime) {
									if (mOutputItems != null)
										for (ItemStack tStack : mOutputItems)
											if (tStack != null) {
												// try{GT_Mod.instance.achievements.issueAchivementHatch(aBaseMetaTileEntity.getWorld().getPlayerEntityByName(aBaseMetaTileEntity.getOwnerName()),
												// tStack);}catch(Exception e){}
												addOutput(tStack);
											}
			...
			aBaseMetaTileEntity.setErrorDisplayID((aBaseMetaTileEntity.getErrorDisplayID() & ~127) | (mWrench ? 0 : 1)
					| (mScrewdriver ? 0 : 2) | (mSoftHammer ? 0 : 4) | (mHardHammer ? 0 : 8) | (mSolderingTool ? 0 : 16)
	...
	public int getRepairStatus() {
		return (mWrench ? 1 : 0) + (mScrewdriver ? 1 : 0) + (mSoftHammer ? 1 : 0) + (mHardHammer ? 1 : 0)
	...
	public boolean doRandomMaintenanceDamage() {
		...
		if (mRuntime++ > 10) {
			...
			if (mInventory[1] != null && getBaseMetaTileEntity().getRandomNumber(2) == 0
				...
				/*
				 * if (mInventory[1].getItem() instanceof
				 * GT_MetaGenerated_Tool_01) { NBTTagCompound tNBT =
				 * mInventory[1].getTagCompound(); if (tNBT != null) {
				 * NBTTagCompound tNBT2 =
				 * tNBT.getCompoundTag("GT.CraftingComponents"); if
				 * (!tNBT.getBoolean("mDis")) { tNBT2 = new NBTTagCompound();
				 * Materials tMaterial =
				 * GT_MetaGenerated_Tool.getPrimaryMaterial(mInventory[1]);
				 * ItemStack tTurbine =
				 * GT_OreDictUnificator.get(OrePrefixes.turbineBlade, tMaterial,
				 * 1); int i = mInventory[1].getItemDamage(); if (i == 170) {
				 * ItemStack tStack = GT_Utility.copyAmount(1, tTurbine);
				 * tNBT2.setTag("Ingredient.0", tStack.writeToNBT(new
				 * NBTTagCompound())); tNBT2.setTag("Ingredient.1",
				 * tStack.writeToNBT(new NBTTagCompound()));
				 * tNBT2.setTag("Ingredient.2", tStack.writeToNBT(new
				 * NBTTagCompound())); tNBT2.setTag("Ingredient.3",
				 * tStack.writeToNBT(new NBTTagCompound())); tStack =
				 * GT_OreDictUnificator.get(OrePrefixes.stickLong,
				 * Materials.Magnalium, 1); tNBT2.setTag("Ingredient.4",
				 * tStack.writeToNBT(new NBTTagCompound())); } else if (i ==
				 * 172) { ItemStack tStack = GT_Utility.copyAmount(1, tTurbine);
				 * tNBT2.setTag("Ingredient.0", tStack.writeToNBT(new
				 * NBTTagCompound())); tNBT2.setTag("Ingredient.1",
				 * tStack.writeToNBT(new NBTTagCompound()));
				 * tNBT2.setTag("Ingredient.2", tStack.writeToNBT(new
				 * NBTTagCompound())); tNBT2.setTag("Ingredient.3",
				 * tStack.writeToNBT(new NBTTagCompound()));
				 * tNBT2.setTag("Ingredient.5", tStack.writeToNBT(new
				 * NBTTagCompound())); tNBT2.setTag("Ingredient.6",
				 * tStack.writeToNBT(new NBTTagCompound()));
				 * tNBT2.setTag("Ingredient.7", tStack.writeToNBT(new
				 * NBTTagCompound())); tNBT2.setTag("Ingredient.8",
				 * tStack.writeToNBT(new NBTTagCompound())); tStack =
				 * GT_OreDictUnificator.get(OrePrefixes.stickLong,
				 * Materials.Titanium, 1); tNBT2.setTag("Ingredient.4",
				 * tStack.writeToNBT(new NBTTagCompound())); } else if (i ==
				 * 174) { ItemStack tStack = GT_Utility.copyAmount(2, tTurbine);
				 * tNBT2.setTag("Ingredient.0", tStack.writeToNBT(new
				 * NBTTagCompound())); tNBT2.setTag("Ingredient.1",
				 * tStack.writeToNBT(new NBTTagCompound()));
				 * tNBT2.setTag("Ingredient.2", tStack.writeToNBT(new
				 ...
				 * tStack.writeToNBT(new NBTTagCompound())); tStack =
				 * GT_OreDictUnificator.get(OrePrefixes.stickLong,
				 * Materials.TungstenSteel, 1); tNBT2.setTag("Ingredient.4",
				 * tStack.writeToNBT(new NBTTagCompound())); } else if (i ==
				 * 176) { ItemStack tStack = GT_Utility.copyAmount(2, tTurbine);
				 * tNBT2.setTag("Ingredient.0", tStack.writeToNBT(new
				 * NBTTagCompound())); tNBT2.setTag("Ingredient.1",
				 * tStack.writeToNBT(new NBTTagCompound()));
				 * tNBT2.setTag("Ingredient.2", tStack.writeToNBT(new
				 ...
				 * tStack.writeToNBT(new NBTTagCompound())); tStack =
				 * GT_OreDictUnificator.get(OrePrefixes.stickLong,
				 * Materials.Americium, 1); tNBT2.setTag("Ingredient.4",
				 * tStack.writeToNBT(new NBTTagCompound())); }
				 ...
				 * mInventory[1].getItem()).doDamage(mInventory[1], (long)
				 * Math.min(mEUt / 5, Math.pow(mEUt, 0.7))); if
				 * (mInventory[1].stackSize == 0) mInventory[1] = null; }
				 */
			}
		}
		return true;
	...
	private void addFluidOutputs(FluidStack[] mOutputFluids2) {
		for (int i = 0; i < mOutputFluids2.length; i++) {
			if (mOutputHatches.size() >= i && mOutputHatches.get(i) != null && mOutputFluids2[i] != null
	...
	public String[] getInfoData() {
		return new String[] { "Progress:", (mProgresstime / 20) + "secs", (mMaxProgresstime / 20) + "secs",
				"Efficiency:", (mEfficiency / 100.0F) + "%", "Problems:", "" + (getIdealStatus() - getRepairStatus()) };
	}
