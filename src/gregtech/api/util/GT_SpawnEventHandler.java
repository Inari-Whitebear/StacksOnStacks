		...
		if (event.entityLiving.isCreatureType(EnumCreatureType.monster, false)) {
			for (int[] rep : mobReps) {
				if (rep[3] == event.entity.worldObj.provider.dimensionId) {
					 ...
					 * instanceof GT_MetaTileEntity_MonsterRepellent){ int r =
					 * ((GT_MetaTileEntity_MonsterRepellent)((BaseMetaTileEntity
					 * )tTile).getMetaTileEntity()).mRange; double dx = rep[0] +
					 * 0.5F - event.entity.posX; double dy = rep[1] + 0.5F -
					 * event.entity.posY; double dz = rep[2] + 0.5F -
					 * event.entity.posZ; if ((dx * dx + dz * dz + dy * dy) <=
					 * Math.pow(r, 2)) { event.setResult(Event.Result.DENY); } }
					 */
				}
			}
		}
