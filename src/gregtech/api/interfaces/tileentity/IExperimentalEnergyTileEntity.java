...
public interface IExperimentalEnergyTileEntity extends IColoredTileEntity, IHasWorldObjectAndCoords {
	...
	public static class Util {
		...
		private static void checkAvailabilities() {
			if (CHECK_ALL) {
				/*
				 * try { Class tClass = cofh.api.energy.IEnergyReceiver.class;
				 * tClass.getCanonicalName(); RF_ENERGY = T; } catch(Throwable
				 * e) {}
				 */
				try {
		...
		/**
		 * Emits Energy to the adjacent Blocks. Also compatible with adjacent
		 * IC2 TileEntities when electric and RF TileEntities when RedstoneFlux.
		 * 
			...
			for (byte i = 0, j = 0; i < 6 && aSecondary > rUsedSecondary; i++)
				if (aEmitter.outputsEnergyTo(aEnergyType, i)) {
					...
					if (tTileEntity instanceof IExperimentalEnergyTileEntity) {
						...
						rUsedSecondary += ((IExperimentalEnergyTileEntity) tTileEntity).injectEnergy(aEnergyType, j,
								aPrimary, aSecondary - rUsedSecondary);
						...
						if (((IEnergySink) tTileEntity).acceptsEnergyFrom((TileEntity) aEmitter,
							...
							while (aSecondary > rUsedSecondary && ((IEnergySink) tTileEntity).getDemandedEnergy() > 0
									&& ((IEnergySink) tTileEntity).injectEnergy(ForgeDirection.getOrientation(j),
											aPrimary, aPrimary) < aPrimary)
								rUsedSecondary++;
						}
					} /*
						 * else if (RF_ENERGY && aEnergyType ==
						 * SubTag.ENERGY_REDSTONE_FLUX && tTileEntity instanceof
						 * IEnergyReceiver && ((IEnergyReceiver)
						 * tTileEntity).canConnectEnergy(ForgeDirection.
						 * getOrientation(j))) { rUsedSecondary +=
						 * ((IEnergyReceiver) tTileEntity)
						 * .receiveEnergy(ForgeDirection.getOrientation(j),
						 * (int) aSecondary, F); }
						 */
				}
