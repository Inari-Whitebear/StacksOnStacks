...
public abstract class GT_MetaGenerated_Item extends GT_MetaBase_Item {
				...
				if (tRandomData != null) {
					...
					if (tRandomData instanceof IFoodStat) {
						...
						if (((IFoodStat) tRandomData).getFoodAction(this, rStack) == EnumAction.eat) {
							...
							if (tFoodValue > 0)
								RA.addCannerRecipe(rStack, ItemList.IC2_Food_Can_Empty.get(tFoodValue),
	 ...
	 * @param aFoodBehavior
	 *            the Food Behavior you want to add.
	...
	public final GT_MetaGenerated_Item setElectricStats(int aMetaValue, long aMaxCharge, long aTransferLimit,
		...
		if (aMaxCharge == 0)
			...
			if (aMetaValue >= mOffset && aUseAnimations)
				mIconList[aMetaValue - mOffset] = Arrays.copyOf(mIconList[aMetaValue - mOffset],
	...
	public final ItemStack onEaten(ItemStack aStack, World aWorld, EntityPlayer aPlayer) {
		...
		if (tStat != null) {
			if (Loader.isModLoaded(MOD_ID_APC)) {
				...
				aPlayer.getFoodStats().addStats(tStat.getFoodLevel(this, aStack, aPlayer),
	...
	// : new FoodValues(tStat.getFoodLevel(this, aStack, null),
	// tStat.getSaturation(this, aStack, null));
	// }
