...
public abstract class GT_MetaGenerated_Tool extends GT_MetaBase_Item implements IDamagableItem {
	...
	public final ItemStack addTool(int aID, String aEnglish, String aToolTip, IToolStats aToolStats,
		...
		if (aID >= 0 && aID < 32766 && aID % 2 == 0) {
			...
			GT_LanguageManager.addStringLocalization(getUnlocalizedName() + "." + (aID + 1) + ".name",
					aEnglish + " (Empty)");
			GT_LanguageManager.addStringLocalization(getUnlocalizedName() + "." + (aID + 1) + ".tooltip",
	...
	public void onHarvestBlockEvent(ArrayList<ItemStack> aDrops, ItemStack aStack, EntityPlayer aPlayer, Block aBlock,
		...
		if (isItemStackUsable(aStack) && getDigSpeed(aStack, aBlock, aMetaData) > 0.0F)
			doDamage(aStack, tStats.convertBlockDrops(aDrops, aStack, aPlayer, aBlock, aX, aY, aZ, aMetaData, aFortune,
					aSilkTouch, aEvent) * tStats.getToolDamagePerDropConversion());
	}

	@Override
	public boolean onLeftClickEntity(ItemStack aStack, EntityPlayer aPlayer, Entity aEntity) {
		IToolStats tStats = getToolStats(aStack);
		if (tStats == null || !isItemStackUsable(aStack))
			return true;
		...
		if (aEntity.canAttackWithItem() && !aEntity.hitByEntity(aPlayer)) {
			...
			if (tDamage + tMagicDamage > 0.0F) {
				boolean tCriticalHit = aPlayer.fallDistance > 0.0F && !aPlayer.onGround && !aPlayer.isOnLadder()
	...
	public void addAdditionalToolTips(List aList, ItemStack aStack) {
		...
		if (tStats != null) {
			...
			if (name.equals("gt.metatool.01.170") || name.equals("gt.metatool.01.172")
				...
				aList.add(tOffset + 0, EnumChatFormatting.WHITE + "Durability: " + EnumChatFormatting.GREEN
						+ (tMaxDamage - getToolDamage(aStack)) + " / " + tMaxDamage + EnumChatFormatting.GRAY);
				aList.add(tOffset + 1, EnumChatFormatting.WHITE + tMaterial.mDefaultLocalName
						+ EnumChatFormatting.YELLOW + " lvl " + getHarvestLevel(aStack, "") + EnumChatFormatting.GRAY);
				aList.add(tOffset + 2, EnumChatFormatting.WHITE + "Turbine Efficency: " + EnumChatFormatting.BLUE
						+ (50.0F + (10.0F * getToolCombatDamage(aStack))) + EnumChatFormatting.GRAY);
				aList.add(tOffset + 3,
				...
				aList.add(tOffset + 2, EnumChatFormatting.WHITE + "Attack Damage: " + EnumChatFormatting.BLUE
						+ getToolCombatDamage(aStack) + EnumChatFormatting.GRAY);
				aList.add(tOffset + 3,
	...
	public final ItemStack getContainerItem(ItemStack aStack) {
		...
		if (aStack == null)
			...
			GT_Utility.doSoundAtClient(tStats.getCraftingSound(), 1, 1.0F);
		return aStack;
	}
	...
	/*
	 * @Override public float getSaplingModifier(ItemStack aStack, World aWorld,
	 * EntityPlayer aPlayer, int aX, int aY, int aZ) { IToolStats tStats =
	 * getToolStats(aStack); return tStats != null && tStats.isGrafter() ?
	 * Math.min(100.0F, (1 + getHarvestLevel(aStack, "")) * 20.0F) : 0.0F; }
	 * 
	 * 
	 * @Override public boolean canWhack(EntityPlayer aPlayer, ItemStack aStack,
	 * int aX, int aY, int aZ) { if (!isItemStackUsable(aStack)) return false;
	 * IToolStats tStats = getToolStats(aStack); return tStats != null &&
	 * tStats.isCrowbar(); }
	 * 
	 * @Override public void onWhack(EntityPlayer aPlayer, ItemStack aStack, int
	 * aX, int aY, int aZ) { IToolStats tStats = getToolStats(aStack); if
	 * (tStats != null) doDamage(aStack, tStats.getToolDamagePerEntityAttack());
	 * }
	 * 
	 * @Override public boolean canLink(EntityPlayer aPlayer, ItemStack aStack,
	 * EntityMinecart cart) { if (!isItemStackUsable(aStack)) return false;
	 * IToolStats tStats = getToolStats(aStack); return tStats != null &&
	 * tStats.isCrowbar(); }
	 * 
	 * @Override public void onLink(EntityPlayer aPlayer, ItemStack aStack,
	 * EntityMinecart cart) { IToolStats tStats = getToolStats(aStack); if
	 * (tStats != null) doDamage(aStack, tStats.getToolDamagePerEntityAttack());
	 * }
	 * 
	 * @Override public boolean canBoost(EntityPlayer aPlayer, ItemStack aStack,
	 * EntityMinecart cart) { if (!isItemStackUsable(aStack)) return false;
	 * IToolStats tStats = getToolStats(aStack); return tStats != null &&
	 * tStats.isCrowbar(); }
	 * 
	 * @Override public void onBoost(EntityPlayer aPlayer, ItemStack aStack,
	 * EntityMinecart cart) { IToolStats tStats = getToolStats(aStack); if
	 * (tStats != null) doDamage(aStack, tStats.getToolDamagePerEntityAttack());
	 * }
	 */
