...
public class GT_Container extends Container {
	...
	/**
	 * Amount of ALL Slots in the GUI including HoloSlots and ArmorSlots, but
	 * excluding regular Player Slots
	 */
	...
	/**
	 * Amount of Slots in the GUI the player can Shift-Click into. Uses also
	 * getSlotStartIndex
	 */
	...
	/**
	 * Override this Function with something like
	 * "return mTileEntity.isUseableByPlayer(aPlayer);"
	 */
	@Override
	...
	public ItemStack slotClick(int aSlotIndex, int aMouseclick, int aShifthold, EntityPlayer aPlayer) {
				...
				if (aSlot != null) {
					...
					if (tTempStack == null) {
						...
						if (var13 == null) {
							...
							if (tTempStack.getItem() == var13.getItem() && var13.getMaxStackSize() > 1
		...
		} else if (aShifthold == 3 && aPlayer.capabilities.isCreativeMode && aPlayerInventory.getItemStack() == null
				&& aSlotIndex >= 0) {
			aSlot = (Slot) this.inventorySlots.get(aSlotIndex);
	...
	public ItemStack transferStackInSlot(EntityPlayer aPlayer, int aSlotIndex) {
		...
		if (getSlotCount() > 0 && slotObject != null && slotObject.getHasStack()
			...
			if (aSlotIndex < getAllSlotCount()) {
				...
				// Player -> TileEntity
			} else if (!mergeItemStack(stackInSlot, getShiftClickStartIndex(),
					getShiftClickStartIndex() + getShiftClickSlotCount(), false)) {
				return null;
			}
	...
	protected boolean mergeItemStack(ItemStack aStack, int aStartIndex, int aSlotCount, boolean par4) {
		...
		if (aStack.isStackable()) {
			while (aStack.stackSize > 0 && (!par4 && var6 < aSlotCount || par4 && var6 >= aStartIndex)) {
				...
				if (!(var7 instanceof GT_Slot_Holo) && !(var7 instanceof GT_Slot_Output) && var8 != null
